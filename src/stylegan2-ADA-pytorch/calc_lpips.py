...
# Number of images to generate per snapshot for comparison. 
# 20 images = 190 unique pairs. This provides a highly stable average while staying fast.
NUM_IMAGES = 20 
...
with open(OUTPUT_CSV, mode='w', newline='') as csv_file:
    ...
    # Using a format that mimics your existing metric JSONs/CSVs
    ...
    for idx, pkl_file in enumerate(pkl_files_to_test, 1):
        ...
        # Extract kimg (e.g., 'network-snapshot-000100.pkl' -> 100)
        ...
        # Generate images
        torch.manual_seed(42) # Fixed seed for fair comparison across snapshots
        ...
        print(f"    ↳ LPIPS Diversity Score: {average_lpips:.4f}")
        
        # Write to CSV immediately so data is saved even if you stop the script early
        csv_writer.writerow([snapshot_name, kimg, average_lpips])
        
        # Free up GPU memory for the next snapshot
        del G
        del img_tensors
