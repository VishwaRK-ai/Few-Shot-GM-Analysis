...
def training_loop(
    ...
    if (augment_kwargs is not None) and (augment_p > 0 or ada_target is not None):
        ...
        augment_pipe.p.copy_(augment_pipe.p.clamp(max=0.3))
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            print(f'\r  phase {phase.name} done', end='', flush=True)
            ...
            for name, module in [('G', G), ('D', D), ('G_ema', G_ema), ('augment_pipe', augment_pipe), ('D_ema', D_ema), ('DHead', DHead), ('GHead', GHead)]:
                if module is not None:
                    if name in ['DHead', 'GHead']:
                        module = module.module if hasattr(module, 'module') else module if hasattr(module, 'module') else module
                    if num_gpus > 1:
