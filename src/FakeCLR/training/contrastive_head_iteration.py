...
class CLHead(torch.nn.Module):
    def __init__(self,
        ...
        self.register_buffer("queue_label", torch.randn(1, self.queue_size))
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue_label(self, keys,label):
        ...
        if batch_size > self.queue_size:
            ...
            self.queue_label[:,0:]=label[:, :self.queue_size]
            ...
            self.queue_label[:, ptr:] = label[:, :self.queue_size - ptr]
            self.queue_label[:, :batch_size - (self.queue_size - ptr)] = label[:, self.queue_size-ptr:]
    ...
    def forward(self, im_q, im_k,batch_idx, loss_only=False, update_q=False):
        """
        Input:
        ...
        tem_label=self.queue_label.clone().detach()
        
        tem_label=tem_label/torch.max(tem_label)#change_label
        #tem_label=(tem_label-torch.min(tem_label))/(torch.max(tem_label)-torch.min(tem_label))#no_change_label
        tem_label=torch.nn.functional.normalize(tem_label,p=2,dim=0)
        tem_label=tem_label/0.01
        t=F.softmax(tem_label,dim=0)
        l_neg=l_neg+t
        ...
        if not loss_only:
            ...
            self._dequeue_and_enqueue_label(k,batch_idx)
