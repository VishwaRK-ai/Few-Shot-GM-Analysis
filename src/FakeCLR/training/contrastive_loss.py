...
class StyleGAN2LossCL(Loss):
    ...
    def run_D(self, img, c, sync, detach_aug=False):
        if self.augment_pipe is not None:
            if detach_aug:
                with torch.no_grad():
                    img = self.augment_pipe(img.detach()).requires_grad_(img.requires_grad)
            else:
    ...
    def run_cl(self, img, c, sync, contrastive_head, D_ema,batch_idx, loss_name='', loss_only=False, img1=None, update_q=False):
        ...
        if self.augment_pipe is not None:
            img0 = self.augment_pipe(img)
            img1 = self.augment_pipe(img) if img1 is None else self.augment_pipe(img1)
        else:
            img0 = img
            img1 = img if img1 is None else img1

        # extract features for two views via D and momentum D
        _, logits0 = self.D(img0, c, return_feats=True)
        with torch.no_grad():
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain,batch_idx, cl_phases=None, D_ema=None, lw_real_cl=1.0, lw_fake_cl=1.0, lw_fake_cl_on_g=1.0, g_fake_cl=False):
        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                real_logits = self.run_D(real_img_tmp, real_c, sync=sync, detach_aug=True)
                ...
                if do_Dr1:
                    with torch.autograd.profiler.record_function('r1_grads'), conv2d_gradfix.no_weight_gradients():
                        r1_grads = torch.autograd.grad(outputs=[real_logits.sum()], inputs=[real_img_tmp], create_graph=False, only_inputs=True, allow_unused=True)[0]
                        if r1_grads is None:
                            r1_grads = torch.zeros_like(real_img_tmp)                   
