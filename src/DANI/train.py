...
@click.option('--subset',       help='Training with only N images', metavar='INT',              type=int, required=True)
...
def main(**kwargs):
    ...
    if opts.subset is not None:
        assert isinstance(opts.subset, int)
        if not 1 <= opts.subset <= c.training_set_kwargs.max_size:
            raise UserError(f'--subset must be between 1 and {args.training_set_kwargs.max_size}')
        if opts.subset < c.training_set_kwargs.max_size:
            c.training_set_kwargs.max_size = opts.subset
