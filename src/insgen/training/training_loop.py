...
import legacy
from metrics import metric_main_ex
...
# Resume from existing pickle.
    if (resume_pkl is not None) and (rank == 0):
        ...
        for name, module in [('G', G), ('D', D), ('G_ema', G_ema), ('D_ema', D_ema), ('DHead', DHead), ('GHead', GHead)]:
            ...
            elif name == 'D_ema' and 'D' in resume_data:
                misc.copy_params_and_buffers(resume_data['D'], module, require_all=False)
    ...
    while True:
        ...
        if (network_snapshot_ticks is not None) and (done or cur_tick % network_snapshot_ticks == 0):
            ...
            for name, module in [('G', G), ('D', D), ('G_ema', G_ema), ('augment_pipe', augment_pipe), ('D_ema', D_ema), ('DHead', DHead), ('GHead', GHead)]:
                if module is not None:
                    if name in ['DHead', 'GHead'] and num_gpus > 1:
        ...
        if (snapshot_data is not None) and (len(metrics) > 0):
            ...
            for metric in metrics:
                result_dict = metric_main_ex.calc_metric(metric=metric, G=snapshot_data['G_ema'],
                    dataset_kwargs=training_set_kwargs, num_gpus=num_gpus, rank=rank, device=device)
                if rank == 0:
                    metric_main_ex.report_metric(result_dict, run_dir=run_dir, snapshot_pkl=snapshot_pkl)
